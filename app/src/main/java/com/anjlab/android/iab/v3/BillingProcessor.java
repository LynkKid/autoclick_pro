...
public class BillingProcessor extends BillingBase
{
	...
	**/
								   ...
								   final String oldProductId)
	{
		...
		handler.post(new Runnable()
		{
			...
			public void run()
			{
				...
				billingService.launchBillingFlow(activity, params).getResponseCode();
			}
		});
	}

	public void handleItemAlreadyOwned(final String productId)
	{
	...
	public boolean mySubscribe(Activity activity, String productId) {
		return myPurchase(activity, null, productId, Constants.PRODUCT_TYPE_SUBSCRIPTION);
	}
	private boolean myPurchase(final Activity activity, final String oldProductId, final String productId,
							 String purchaseType)
	{
		if (!isConnected() || TextUtils.isEmpty(productId) || TextUtils.isEmpty(purchaseType))
		{
		...
		try
		{
			...
			productSubsList.add(QueryProductDetailsParams.Product.newBuilder().setProductId(productId).setProductType(SUBS).build());
			QueryProductDetailsParams productDetailsParams = QueryProductDetailsParams.newBuilder().setProductList(productSubsList).build();
			billingService.queryProductDetailsAsync(productDetailsParams, (billingResult, productDetailsList) -> {
				if (billingResult.getResponseCode() == OK) {
					if (productDetailsList.isEmpty()) {
						...
						BillingResult res = billingService.launchBillingFlow(activity, billingFlowParams);
						if (res.getResponseCode() == BillingClient.BillingResponseCode.ITEM_ALREADY_OWNED)
						{
