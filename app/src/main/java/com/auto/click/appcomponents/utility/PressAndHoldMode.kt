...
class PressAndHoldMode(
    ...
    private fun addViewClick(modelAddViewForMultiClick: ModelAddViewForMultiClick? = null) {
        ...
        listViewClick.add(
            ModelAddViewForMultiClick(
                ...
                timeOfPressAndHold = modelAddViewForMultiClick?.timeOfPressAndHold
                    ?: PreferenceHelper.getInt(
                        Contact.TIME_PRESS_AND_HOLD_DEFAULT,
                        300
                    ),
                isPressAndHold = true
            )
            ...
            listViewClick.forEach { element ->
                configMultiAutoClickModel.listClick.add(
                    ModelAddViewForMultiClick(
                        ...
                        timeOfPressAndHold = element.timeOfPressAndHold,
                        isClick = element.isClick,
                        isPressAndHold = element.isPressAndHold
                    )
                )
            }
            val dataListConfigSave = Utils.getItemDataList()
    ...
    private fun showPopupSettingClick(element: ModelAddViewForMultiClick) {
        ...
        popupSettingClick.onDoneAction =
            { optionDelay, delayTime, optionDelayClickOfNext, delayTimeClickOfNext, repeatNum, swipeTime, pressAndHoldTime ->
                ...
                element.timeOfPressAndHold = (pressAndHoldTime ?: "1").toInt()
            }
    ...
    private fun setupViewParams(modelAddView: ModelAddViewForMultiClick) {
        ...
        timeIntervalReplayCyclesAll += (modelAddView.getTotalTimeIntervalClickOfNext() * modelAddView.numberOfCycles) + modelAddView.getTotalTimeInterval() + modelAddView.timeOfPressAndHold
    }
    ...
    private suspend fun performClick(index: Int) {
        if (index < listViewClick.size) {
            ...
            val totalDelayBeforeStart = modelAddView.getTotalTimeInterval()
            val timeHold = modelAddView.timeOfPressAndHold.toLong()
            val delayBetween = timeIntervalReplay + timeHold

            delay(totalDelayBeforeStart)

            repeat(modelAddView.numberOfCycles) {
                if (timerCyclesAll != null) {
                    autoClickService?.longPressAndHold(
                        modelAddView.locationPointsModelStart!!,
                        configMultiAutoClickModel.antiDetection,
                        timeHold
                    )
                    delay(delayBetween)
                } else {
                    return // dừng nếu bị stop
                }
            }
        }
    }
