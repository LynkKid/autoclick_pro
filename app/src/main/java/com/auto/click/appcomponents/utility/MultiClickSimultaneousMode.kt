...
class MultiClickSimultaneousMode(
    ...
    private fun getAddTouchListener(params: WindowManager.LayoutParams) = TouchAndDragListener(
        params, startDragDistance,
        {
            if (listViewClick.size >= 10) return@TouchAndDragListener
            addViewClick()
            listViewClick.forEach {
                if (!it.isAdded) {
                    manager.addView(it.viewStart, it.paramLayoutStart)
    ...
    private fun showPopupSettings() {
        ...
        popupSettings.onDoneAction =
            { nameConfig: String, timeDelay: Int, optionDelay: Int, optionSelected: Int, hours: Int, minutes: Int, seconds: Int, numberOfCycler: Int, antiDetect: Boolean ->
                configMultiAutoClickModel.nameConfig = nameConfig
                configMultiAutoClickModel.optionDelay = optionDelay
                configMultiAutoClickModel.timerDelay = timeDelay
    ...
    private fun startClickingActions() {
        ...
        val duration = if (configMultiAutoClickModel.antiDetection) PreferenceHelper.getInt(
            Contact.SETTINGS_ANTI_DETECT_TIME,
            0
        ) else 0
        listViewClick.forEach { setupViewParams(it) }
        timeIntervalReplay = configMultiAutoClickModel.getTotalTimeIntervalClickOfNext()
        val delay = 200L
        timerCyclesAll =
    ...
    private fun ConfigMultiAutoClickModel.getTotalTimeIntervalClickOfNext(): Long {
    ...
    private fun startPerformClick() {
        ...
        if (configMultiAutoClickModel.optionSelected == 2 && countNumberOfCycles > configMultiAutoClickModel.numberOfCycles) {
            stopClickingActions().also { isOn = !isOn }
            enableButton(true)
            visibleButton(true)
        } else {
            val positions = listViewClick.mapNotNull { it.locationPointsModelStart }
            autoClickService?.multiClickSimultaneous(
                positions,
